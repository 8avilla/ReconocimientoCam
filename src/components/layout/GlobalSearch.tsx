...
/** Search button for the top bar: finds teams, players and matches of the active championship. */
export function GlobalSearch({ championshipId }: { championshipId: string }) {
  ...
  return (
    <>
      ...
      {open && <SearchModal championshipId={championshipId} onClose={() => setOpen(false)} />}
    </>
  );
}
